...
@dp.message_handler(Command("start"), state="*") async def start_cmd(m: types.Message, state: FSMContext): await state.finish() await m.answer("👋 Bienvenido. Usa /permiso para iniciar")
...
@dp.message_handler(state=PermisoForm.nombre) async def get_nombre(m: types.Message, state: FSMContext): datos = await state.get_data() datos["nombre"] = m.text.strip() datos["folio"] = nuevo_folio()

try:
    path1 = generar_pdf_principal(datos)
    path2 = generar_pdf_bueno(datos["serie"], datetime.now(), datos["folio"])

    await m.answer_document(open(path1, "rb"), caption=f"📄 Principal - Folio: {datos['folio']}")
