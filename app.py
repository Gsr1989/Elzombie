...
PLANTILLA_PDF = "cdmxdigital2025ppp.pdf"  # asegúrate de incluir este archivo en el repo
...
def subir_pdf_supabase(path_local: str, nombre_pdf: str) -> str:
    try:
        ...
        logger.exception(f"Fallo subiendo a Supabase: {e}")
        return ""
...
def guardar_supabase(data: dict):
    try:
        ...
        logger.exception(f"Fallo insert en Supabase: {e}")

def generar_folio_automatico(prefijo: str = "05") -> str:
    try:
        resp = supabase.table("borradores_registros").select("folio").order("folio", desc=True).limit(1).execute()
        if resp.data:
            ultimo = resp.data[0]["folio"]
            try:
        ...
        logger.exception(f"No pude consultar folio en Supabase: {e}")
        num = 1
...
def _make_pdf(datos: dict) -> str:
    ...
    # Texto de fecha (respeta tu formato “mes” visual)
    meses_es = {
    ...
    mes = meses_es[fecha_exp.strftime("%B")]
    fecha_visual = f"{fecha_exp.strftime('%d')} DE {mes} DEL {fecha_exp.strftime('%Y')}"
    ...
    # Colocar QR (coordenadas que usas)
    tam_qr = 1.6 * 28.35
...
async def form_marca(m: types.Message, state: FSMContext):
    ...
    await bot.send_message(m.chat.id, "Línea del vehículo:")
...
async def form_linea(m: types.Message, state: FSMContext):
    ...
    await bot.send_message(m.chat.id, "Año:")
...
async def form_anio(m: types.Message, state: FSMContext):
    ...
    await bot.send_message(m.chat.id, "Serie (VIN):")
...
async def form_serie(m: types.Message, state: FSMContext):
    ...
    await bot.send_message(m.chat.id, "Motor:")
...
async def form_motor(m: types.Message, state: FSMContext):
    ...
    await bot.send_message(m.chat.id, "Nombre del solicitante:")
...
async def form_nombre(m: types.Message, state: FSMContext):
    ...
    try:
        ...
        await bot.send_message(m.chat.id, f"❌ Error generando PDF: {e}")
        await state.finish()
        return
    ...
    try:
        with open(path_pdf, "rb") as f:
            await bot.send_document(m.chat.id, f, caption=caption)
    except Exception as e:
        await bot.send_message(m.chat.id, f"PDF listo pero no pude enviarlo: {e}\nURL: {url_pdf or '—'}")
...
async def telegram_webhook(request: Request):
    ...
    try:
        ...
        logger.exception(f"Parse Update falló: {e}")
        return {"ok": True, "note": "parse_failed"}

    try:
        # Asegura contexto actual tras wake-up en Render
        Bot.set_current(bot)
    ...
    return {"ok": True}supabase: Client = create_client(SUPABASE_URL, SUPABASE_KEY)
