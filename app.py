...
```
    ...
    for row in r.data or []:
        f = str(row.get("folio", ""))
        if f.startswith(FOLIO_PREFIJO) and f[len(FOLIO_PREFIJO):].isdigit():
            existentes.add(int(f[len(FOLIO_PREFIJO):]))

except Exception as e:
    print(f"[ERROR] Cargando folios existentes: {e}")
    existentes = set()
...
raise Exception("Sin folio disponible — límite alcanzado")
```
...
```
    ...
    if consecutivos:
        ...
        print(f"[INFO] Folio inicializado — máximo: {FOLIO_PREFIJO}{maximo} → siguiente: {folio_counter['siguiente']}")
    else:
        folio_counter["siguiente"] = 1
...
f”Use /chuleta para generar otro permiso.”)
else:
